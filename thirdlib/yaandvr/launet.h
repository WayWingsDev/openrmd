...
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientStartup(UINT m_nMessage,HWND m_hWnd,
		void (WINAPI *m_messagecallback)(LONG hHandle,int wParam,
			int lParam,void *context) /*= NULL*/,
		void *context /*= NULL*/, char *key /*= NULL*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientReadMessage(char *m_sername,
		char *m_url,WORD *m_port,int *m_ch,DWORD *wParam,DWORD *lParam);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientWaitTime(int m_waitnum /*= 6*/,
		int m_trynum /*= 3*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientCleanup();

//connect server,record
CNET_APIIMPORT LONG  __stdcall VSNET_ClientStart(char *m_url,
		CHANNEL_CLIENTINFO *m_pChaninfo,WORD wserport /*= 3000*/,
		int streamtype /*= 0*/);
...
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientRefrenshWnd(LONG hHandle,
		RECT *rect /*= NULL*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientShowcallback(LONG hHandle,
		void(WINAPI *ShowCallBack)(BYTE *m_y,BYTE *m_u,BYTE *m_v,
			int stridey,int strideuv,int width,int height,
			void *context),void *context);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientAudioDeccallback(LONG hHandle,
		void(WINAPI *AudioDecCallBack)(char *pBuffer,int size,
			void *context),void *context);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientDrawCallBack(LONG hHandle,
		void(WINAPI *DrawCallBack)(HDC hDC,void *context),
		void *context);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientVideoQuality(LONG hHandle,
		int m_quality);
CNET_APIIMPORT LONG  __stdcall VSNET_ClientGetState(LONG hHandle);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientStartASFFileCap(LONG hHandle,
		LPTSTR m_FileName,BOOL m_bbroad /*= FALSE*/);
...
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientGetVideoSize(LONG hHandle,
		DWORD *m_pWidth,DWORD *m_pHeight);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientCapturePicture(LONG hHandle,
		LPTSTR m_filename);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientPTZCtrl(LONG hHandle,int type,
		int value,int priority /*= 0*/, char *extrabuff /*= NULL*/, 
		int extrasize /*= 0*/);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientSetPTZAddr(LONG hHandle,
		BYTE m_ptzaddr);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientPTZDev(LONG hHandle,
		char m_devch,BOOL bOn);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientOutPut(LONG hHandle,
		char m_devch,BOOL bOn);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientViAdjust(LONG hHandle,
		BYTE m_bri,BYTE m_con,BYTE m_sta,BYTE m_hue);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientSwitchChannel(LONG hHandle,
		char m_curch);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientPrerecord(LONG hHandle,
		BOOL m_benable,int m_buffsize,int m_framecount);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSerialSend(LONG hHandle,
		int m_serialport,BYTE *pData,int size);
...
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetStreamInfo(LONG hHandle,
		VSTREAMINFO *m_pStreamInfo);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientStartNomalCap(LONG hHandle,
		void(WINAPI *m_nomalvideo)(char *pbuff,int headsize,
			int datasize,int timetick,int biskeyframe,
			void *context),void *pvideocontext,
		void(WINAPI *m_nomalaudio)(char *pbuff,int headsize,
			int datasize,int timetick,int biskeyframe,
			void *context),void *paudiocontext);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientStopNomalCap(LONG hHandle);
CNET_APIIMPORT LONG  __stdcall VSNET_InfoClientStart(char *m_infourl,
		CHANNEL_CLIENTINFO *m_pChaninfo,WORD winfoport /*= 8080*/,
		int streamtype /*= 0*/);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientInfoGetIPHandle(LONG hHandle,
		char* m_pgetip,WORD *m_wgetport);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientStartMp4Capture(LONG hHandle, 
		LPTSTR m_FileName);
...
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientCapturePictureEx(LONG hHandle,
		char *pbuff,int stride,int size);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientSetPriority(LONG hHandle,
		int priority);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientSendBankInfo(LONG hHandle,
		NETRVS_BANKINFO *pbankinfo);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientFireExtendAlarm(LONG hHandle,
		int alarmType,float alarmValue);

//talk and brodcast(auto collection)
CNET_APIIMPORT LONG  __stdcall VSNET_ClientTalkExchangeStart(char *m_url,
		char *m_username,char *m_password,WORD wserport /*= 3000*/,
		void(WINAPI *TalkCallBack)(char *pbuff,int size) /*= NULL*/,
		char *m_servername /*= NULL*/, int samplerate /*= 8000*/);
...
CNET_APIIMPORT int   __stdcall VSNET_ClientTalkAddnew(char *m_url,
		char *m_username,char *m_password,WORD wserport /*= 3000*/,
		char *m_servername /*= NULL*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientTalkDel(char *m_url,
		WORD wserport /*= 3000*/,char *m_servername /*= NULL*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientTalkBroadcastStop();
//talk(user collection)
CNET_APIIMPORT int   __stdcall VSNET_ClientTalkStart(char *m_url,
		char *m_username,char *m_password,int m_tranType,
		LONG *handle,WORD wserport/*=3000*/,
		char *m_servername /*= NULL*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientTalkStop(LONG hAudioHandle);
CNET_APIIMPORT int __stdcall VSNET_ClientTalkSendData(LONG hAudioHandle,
		char *pBuf,int buflen);
CNET_APIIMPORT void  __stdcall VSNET_ClientTalkFreeAll();

//update server
CNET_APIIMPORT LONG  __stdcall VSNET_ClientUpdateStart(char *m_url,
		char *m_username,char *m_password,char *m_filename,
		HANDLE m_hEndEvent,WORD wserport /*= 3000*/,
		char *m_sername /*= NULL*/);
CNET_APIIMPORT int   __stdcall VSNET_ClientUpdateStop(LONG hHandle);
CNET_APIIMPORT void	 __stdcall VSNET_StartUploadRom(char *m_sername,
		char *m_url,char *m_username,char *m_password,WORD m_wserport, 
		char* m_szRomPath);
CNET_APIIMPORT int	 __stdcall VSNET_GetUploadRomRst();

//param function
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientClearAlarmOut(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetChannelAmount(char *m_sername,
		char *m_url,DWORD *m_channs,DWORD *m_alarmins,
		DWORD *m_alarmouts,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetConnectIP(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		CONNECTIP *m_pConnecIP,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientResetServer(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientShutDownServer(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetWholePara(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		WHOLEPARAM *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetWholePara(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		WHOLEPARAM *m_pSetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetChannelPara(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNELPARAM *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetChannelPara(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNELPARAM *m_pSetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetAlarmRecord(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		ALARMPARAM *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetAlarmRecord(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		ALARMPARAM *m_pSetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetMotionPara(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNMOTION *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetMotionPara(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNMOTION *m_pSetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientMotionTest(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		BOOL m_bOn,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetVideoMask(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNVIDEOMASK *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetVideoMask(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNVIDEOMASK *m_pSetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetChanOSD(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNOSD *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetChanOSD(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNOSD *m_pSetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetChanOSDWithBK(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNOSD *m_pGetPara,WORD wserport /*= 3000*/, 
		char *m_szOSDTransparent /*= NULL*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetChanOSDWithBK(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNOSD *m_pSetPara,WORD wserport /*= 3000*/, 
		char *m_szOSDTransparent /*= NULL*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSaveServerPara(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientDefaultPara(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetViparam(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNVIPARAM *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetViparam(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNVIPARAM *m_pSetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetSerial485Param(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		SERIAL485_PARAM *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetSerial485Param(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		SERIAL485_PARAM *m_pSetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetSystime(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		TIME_PARAM *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetSystime(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		TIME_PARAM *m_pSetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetSerialNo(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		BYTE m_serial[50],WORD wserport /*= 3000*/);
CNET_APIIMPORT int   __stdcall VSNET_ClientAffirmUser(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetUser(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		WHOLE_USER *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetUser(char *m_sername,char *m_url,
		char *m_username,char *m_password,WHOLE_USER *m_pSetPara,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientOutPutServer(char *m_sername,
		char *m_url,char *m_username,char *m_password,char m_devch,
		BOOL bOn,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL __stdcall VSNET_ClientSerialSendToServer(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		int m_serialport,BYTE *pData,int size,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL __stdcall VSNET_ClientGetOutputStatus(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		BYTE m_outputstatus[8],WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL __stdcall VSNET_ClientGetExDeviceStatus(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		BYTE m_outputstatus[16],WORD wserport /*= 3000*/);
CNET_APIIMPORT int __stdcall VSNET_ClientSetPTZCommandFile(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		char *m_filename,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetPTZCommandName(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		char *m_ptzname,WORD wserport /*= 3000*/);
CNET_APIIMPORT int __stdcall VSNET_ClientGetCurSwitchChannel(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetVideoLost(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		BOOL bEnabled,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetVideoLost(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		BOOL *pbEnabled,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetTimerReset(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		VSTIMERRESET *m_pSetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetTimerReset(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		VSTIMERRESET *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetHideAlarm(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNELHIDEALARM *m_pSetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetHideAlarm(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		CHANNELHIDEALARM *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientEncKeyFrame(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		WORD wserport/*=3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetUdpPort(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		unsigned short *videoport,unsigned short *audioport,
		WORD wserport/*=3000*/);
CNET_APIIMPORT int   __stdcall VSNET_ClientServerTalk(ULONG m_nurl,
		char *m_username,char *m_password,int m_tranType,BOOL isStart,
		UINT_PFW *sock,DWORD wserport /*= 3000*/,
		char *m_servername /*= NULL*/);
CNET_APIIMPORT int 	 __stdcall VSNET_ClientSetLicenseFile(char *m_sername,
		char *m_url,char *m_username,char *m_password,char *pbuff,
		int size,WORD wserport /*= 3000*/);
CNET_APIIMPORT int 	 __stdcall VSNET_ClientGetLicenseFile(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		void(WINAPI *licdatacallback)(char *pbuff,int size,
			void *userdata),void *userdata /*= NULL*/,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetHostNotify(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		VSNETHOSTNOTIFY *m_pSetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetHostNotify(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		VSNETHOSTNOTIFY *m_pGetPara,WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL __stdcall VSNET_ClientSetSerialNumData(char *m_sername,
		char *m_url,char *m_username,char *m_password,char *pSetbuff,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL __stdcall VSNET_ClientGetSerialNumData(char *m_sername,
		char *m_url,char *m_username,char *m_password,char *pGetbuff,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL __stdcall VSNET_ClientSetSerialNumPass(char *m_sername,
		char *m_url,char *m_username,char *m_password,char *pSetbuff,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetTempData(char *m_sername,
		char *m_url,char *m_username,char *m_password,char *pbuff,
		int size,WORD wserport /*= 3000*/);
CNET_APIIMPORT int   __stdcall VSNET_ClientGetTempData(char *m_sername,
		char *m_url,char *m_username,char *m_password,char *pbuff,
		WORD wserport /*= 3000*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetPTZText(char *m_sername,
		char *m_url,char *m_username,char *m_password,char *pbuff,
		int size,WORD wserport /*= 3000*/);
CNET_APIIMPORT int	 __stdcall VSNET_ClientPTZLock(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		BOOL block,int priority,WORD wserport/*=3000*/);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ClientSetPTZLockTime(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		int time,WORD wserport/*=3000*/);
CNET_APIIMPORT int __stdcall VSNET_ClientCheckPTZPriority(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		int priority,WORD wserport/*=3000*/);

//remote host
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetRemoteHost(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		VSREMOTEHOST *m_pGetPara,WORD wserport );
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetRemoteHost(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		VSREMOTEHOST *m_pSetPara,WORD wserport );

//sub channel video param
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetSubChanPara(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,VSSUBCHANPARAM *m_pGetPara,WORD wserport );
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientSetSubChanPara(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,VSSUBCHANPARAM *m_pSetPara,WORD wserport );
...
CNET_APIIMPORT BOOL	 __stdcall VSNET_StopServerRecord(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		WORD wserport,VSNETPLAY_FILE_DATA *filedata /*= NULL*/);
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetRecordStatus(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		WORD wserport);
CNET_APIIMPORT BOOL	 __stdcall VSNET_ServerCapJPEG(char *m_sername,
		char *m_url,char m_ch,char *m_username,char *m_password,
		int m_quality,WORD wserport );

//server disk
CNET_APIIMPORT BOOL	 __stdcall VSNET_GetDiskState(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		VSDISKSTATE *m_pDiskState,WORD wserport );
CNET_APIIMPORT int 	 __stdcall VSNET_FormatDisk(char *m_sername,
		char *m_url,char *m_username,char *m_password,int m_diskno,
		WORD wserport );
//server param common
CNET_APIIMPORT LONG	 __stdcall VSNET_ClientMessageOpen(char *sername,
		char *url,char *username,char *password,
		WORD serport /*= 3000*/);
CNET_APIIMPORT int	 __stdcall VSNET_ClientMessageOpt(LONG hHandle,
		int opt,int ch /*= 0*/,void *param1 /*= NULL*/,
...
//talk bothway
CNET_APIIMPORT LONG  __stdcall VSNET_ClientTalkBothwayStart(char *m_url,
		char m_ch,char *m_username,char *m_password,WORD wserport,
		void(WINAPI *TalkCallBack)(char *pbuff,int size),
		char *m_servername,HWND hwnd, int samplerate /*= 8000*/);
...
//get web rom version
CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetWebRomVer(char *m_sername,
		char *m_url,char *m_username,char *m_password,
		VERSIONWEBROM *m_pGetPara,WORD wserport /*= 3000*/);

//sewage
CNET_APIIMPORT BOOL  __stdcall VSNET_FlowMeterComm(char *m_sername, 
		char *m_url, char *m_username,char *m_password,WORD wserport, 
		char comm, int address, VSNETYWFLOWMETERCODE *filedata);
CNET_APIIMPORT BOOL  __stdcall VSNET_SZCYQComm(char *m_sername, char *m_url, 
		char *m_username,char *m_password,WORD wserport, char comm, 
		VSNETYWSZCYQCODE *filedata);
CNET_APIIMPORT BOOL  __stdcall VSNET_SwitchComm(char *m_sername, char *m_url, 
		char *m_username,char *m_password,WORD wserport, char comm);

CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetMsgBuff(LONG hHandle, int type, 
		char *msgbuff);

CNET_APIIMPORT BOOL  __stdcall VSNET_ClientGetVideoAudioInfo(LONG hHandle, 
		CHANNEL_VIDEOINFO *pVideoInfo, CHANNEL_AUDIOINFO *pAudioInfo);
CNET_APIIMPORT int	 __stdcall VSNET_ClientGetVolumeLevel(LONG hHandle);

CNET_APIIMPORT BOOL __stdcall VSNET_ClientRecordIFrameCallback(LONG hHandle,
		pRecordIFrameCallback piframecallback, void *context);

/* Add by Fangxm */
#define VSNET_VIDEO_DATA	1
#define VSNET_AUDIO_DATA	2

typedef void (*data_callback_t)(unsigned long handle, int type, 
