...
#define DH_ENO			CLIENT_GetLastError()
...
{
	printf("The DVR(%s:%ld) disconnect!\n", ip, port);
}
...
static int login_dvr(char *ip, int port, char *user, char *pwd, void *param)
{
	...
	login_id = CLIENT_Login(ip, port, user, pwd, &dev_info, &err);
	if (login_id == 0) {
		syslog(LOG_DEBUG, "CLIENT_Login() error:%d", err);
		return -1;
	}
	syslog(LOG_DEBUG, "CLIENT_Login() success: %d", login_id);
	return login_id;
}
...
static int logout_dvr(void *data, void *param)
{
	...
	if (CLIENT_Logout(login_id) == 0) {
		syslog(LOG_DEBUG, "CLIENT_Logout() error:%d", DH_ENO);
		return -1;
	}
	syslog(LOG_DEBUG, "CLIENT_Logout() success: %d", login_id);
	return 0;
}
...
//			info->param2, info->param3, stop);
	...
	ret = CLIENT_DHPTZControl(login_id, channel_id - 1, info->ctl_code,
...
int special_init()
{
	if (CLIENT_Init(disconn_cb, 0) == 0)
		return -1;

	CLIENT_SetAutoReconnect(reconn_cb, 0);
...
unsigned long special_start_realplay(realplay_info_t *info)
{
	...
	if (strcmp(info->stream_opt, STREAMOPT_MAIN) == 0)
		stream_opt = DH_RType_Realplay_0;
	else
		stream_opt = DH_RType_Realplay_1;

	handle->realplay_handle = CLIENT_RealPlayEx(login_id, 
			info->channel_id - 1, NULL, stream_opt);
	if (handle->realplay_handle == 0) {
		syslog(LOG_DEBUG, "CLIENT_RealPlayEx error:%ld", DH_ENO);
		goto err;
	}
	if (CLIENT_SetRealDataCallBackEx(handle->realplay_handle, 
				realdata_callback, (DWORD)handle, 0x1f) == 0) {
		syslog(LOG_DEBUG, "CLIENT_SetRealDataCallBackEx() error:%ld", 
				DH_ENO);
		goto err;
	}
...
int special_stop_realplay(unsigned long handle)
{
	...
	CLIENT_StopRealPlayEx(p->realplay_handle);
